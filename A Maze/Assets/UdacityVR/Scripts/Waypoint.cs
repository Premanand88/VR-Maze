...
public class Waypoint : MonoBehaviour
{
    ...
    animateCloud checkRun = new animateCloud();
    int index1;
    ...
    void SetColor()
    {
        ...
        if(index == 0)
        {
            currentColor = color_hilight[1];
            index = 1;
        }
        else
        {
            currentColor = color_hilight[0];
            index = 0;
        }
    }
    ...
    private void Start()
    {
        InvokeRepeating("SetColor", 0, 1.0F);
    }
	...
	public void Click()
	{
        ...
        else if (gameObject.tag != "Onfire")
        {
            if (currentColor == color_hilight[1] && !checkRun.IsRun())
            {
                Camera.main.transform.parent.transform.position = WayPoints[Random.Range(0, WayPoints.Length)].transform.position;//gameObject.transform.position;
            }
            else if (currentColor == color_hilight[0] || checkRun.IsRun())
            {
                Camera.main.transform.parent.transform.position = gameObject.transform.position;
            }
        }
	...
	private void Idle()
	{
		...
		Color color				= Color.Lerp(_color_origional, 	  color_hilight[0], _animated_lerp);
	...
	public void Focus()
	{
        ...
        Color color;
        if (checkRun.IsRun() == false) { color = currentColor; }
        else
        {
            color = _color_origional;
        }
        //Color.Lerp(   _color_origional, currentColor, _animated_lerp);

		_scale					= Mathf.Lerp(_scale, scale, lerp_focus);
        _color = color;//Color.Lerp(_color, color,	lerp_focus);
	}
	...
	{	
		...
		_color					= Color.Lerp(_color,     color_hilight[0], lerp_clicked);
	}
