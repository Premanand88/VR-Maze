...
public class ExampleFlocking : MonoBehaviour 
{
	...
	void Start () 
	{
        //_center = cameraPosition.position;
        //initialize the flock array of gameobjects 
        for (int i = 0; i < _flock.Length; i++)
        {
            _flock[i] = Instantiate(flockObject);//GameObject.CreatePrimitive(PrimitiveType.Cube);
	...
	void UpdateFlock(GameObject current_object, GameObject[] flock)
	{
        ...
        center_position = cameraPosition.position+ Random.insideUnitSphere*20.0f;//center_position / neighbor_count;
        //center_position = cameraPosition.position / neighbor_count;
	...
	void OnDrawGizmos()
	{
		if(_selection != null && draw_radius)
		{
            ...
            Gizmos.DrawWireSphere(cameraPosition.position, bounds);
        }
	}
	#endif
}
