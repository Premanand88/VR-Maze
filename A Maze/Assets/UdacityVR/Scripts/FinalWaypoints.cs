...
public class FinalWaypoints : MonoBehaviour
{
	private enum State
	{
		Idle,
    ...
    public Transform templeWayPoint;
    ...
    private void Start()
    {
        InvokeRepeating("SetColor", 0, 0.3F);
    }
    ...
    public void Click()
    {
        if (checkRun.IsRun())
        {
            ...
            if(gameObject.tag == "ToTemple" && templeWayPoint)
            {
                Camera.main.transform.parent.transform.position = templeWayPoint.transform.position;
            }            
        }
	...
	private void Idle()
	{
		...
		Color color				= Color.Lerp(_color_origional, color_hilight.Length>0?color_hilight[0] : _color_origional, _animated_lerp);
